...
public class ExternalPathMovement extends MovementModel {
	...
	private void init() {
		...
		ExternalPathMovementReader reader = ExternalPathMovementReader.getInstance(this.traceFile, this.activeFile);
		this.paths = reader.getPaths(getHost().getAddress());
		this.active = reader.getActive(getHost().getAddress());
		
		//agregado
		int nroInt= getHost().getInterfaces().size();
		for(int i=1;i<=nroInt;i++){
			ActivenessHandler ahOfInterface= host.getInterface(i).getAh();
			double []Times;
			int sizeActiveTime= this.active.size();
			Times=new double[sizeActiveTime*2];
			for(int j=0;j<sizeActiveTime;j++) {
				Times[j*2]=this.active.get(j).start;
				Times[(j*2)+1]=this.active.get(j).end;
			}
			host.getInterface(i).setAh(new ActivenessHandler(Times)); 
		}		
		/////////////////////////////
	}
