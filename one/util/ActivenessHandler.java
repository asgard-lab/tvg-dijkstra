...
public class ActivenessHandler {
	...
	// agregado
	public ActivenessHandler(double[] times) {
		if (times.length % 2 != 0) {
			throw new SettingsError("Invalid amount of values (" + times.length + ") for activity times");
		}
		this.activeTimes = new LinkedList<TimeRange>();
		...
		for (int i = 0; i < times.length; i += 2) {
			...
			if (start > end) {
				throw new SettingsError("Start time cannot be greater than end time");
			}

			timesList.add(new TimeRange(start, end));
		}
		this.activeTimes = timesList;
		if (activeTimes != null) {
			this.curRange = activeTimes.poll();
		}
	}
