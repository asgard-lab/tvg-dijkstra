...
- Usar por commandline, sendo que para executar o programa, fazemos:
  python Convert_EventLog_to_GraphViz.py <arquivo_de_entrada_com_o_eventlog> <rótulo_a_ser_usado_nos_nós> <total_de_nós>
  - O rótulo dos nós é aquela string que escolhemos pra aparecer em cada node, por exemplo "n", então teríamos
    n0, n1, n2, ...
  - O total de nós é o total que vamos utilizar no default_settings, sempre começando do id 0 até total-1,
    por exemplo, se vou usar um total de 10 nós (Algo no default_settings.txt como Group.nrofHosts = 10), teria ids,
    com rótulo por exemplo, n0 até n9.
