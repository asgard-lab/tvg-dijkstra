...
--> Remover coordenadas negativas, por exemplo, se sei que a menor posição X é -5.0 e a menor Y é -10.0, somo
	5.0 às posições X de todo mundo e somo 10.0 às posições Y de todo mundo, dessa maneira, quem tinha as posições
	negativas passará a ter as posições (0,0). Isto é para evitar trabalhar com coordenadas negativas!
...
terminate_annoying_node = False # Até passar a primeira iteração isto aqui está falso, pois não tem ninguém para matar!
temp_time = "Só existem 3 coisas que você só faz uma vez na vida: NASCER, MORRER e VOTAR NO PT!"
temp_id = -9999999 # Nunca que algum nó ali da lista teria um id negativo e ainda deste tamanho!
for index, element in enumerate(sorted_file_list): # A princípio iniciamos as variáveis temp acima com valores absurdos que ninguém no trace nunca teria!
	if ((temp_id == int(element[1])) and (temp_time == str(element[0]))): # Se ocorrer do nó "anterior" (Ou seja, seus valores salvos nas var temp) baterem com os valores do nó atual, é pq o nó atual é repetido. Pode mandar matá-lo!
		terminate_annoying_node = True # Como descobriu que os valores do nó "anterior" batem com os deste nó, é porque tem que mandar matá-lo!
	...
	if (terminate_annoying_node == True):
		del sorted_file_list[index]
		terminate_annoying_node = False
# Agora temos uma lista refinada, onde para cada nó, deletamos as ocorrências iguais de tempo, pra não confundinr o TheONE.


# Obtendo a menor coordenada X e menor coordenada Y do Trace (Procedimento para tratar de coordenadas negativas)
# Só aplicamos este procedimento CASO alguma coordenada encontrada seja negativa.
# Dessa maneira ignora boa parte das coordenadas, que sejam positivas, pois as problemáticas são as negativas!
smallest_X_pos = 0.0
smallest_Y_pos = 0.0
found_negative_X_pos = False
found_negative_Y_pos = False
for element in sorted_file_list:
	if (float(element[4]) < 0.0): # Verificação se a coordenada encontrada X é negativa
		found_negative_X_pos = True # Achou pos. X negativa? Vai ter que reajustar a posição X de TODO MUNDO! O choro é livre!
		if (float(element[4]) < smallest_X_pos): # Se a pos. X negativa for menor que a menor pos. X atual...
			smallest_X_pos = float(element[4]) # Atualiza a nova menor posição X

	if (float(element[5]) < 0.0): # Verificação se a coordenada encontrada Y é negativa
		found_negative_Y_pos = True # Achou pos. Y negativa? Vai ter que reajustar a posição Y de TODO MUNDO! O choro é livre!
		if (float(element[5]) < smallest_Y_pos): # Se a pos. Y negativa for menor que a menor pos. Y atual...
			smallest_Y_pos = float(element[5]) # Atualiza a nova menor posição Y
smallest_X_pos = abs(smallest_X_pos) # Depois de finalmente achar a menor pos. X, transforma em módulo pra somar direito depois.
smallest_Y_pos = abs(smallest_Y_pos) # Depois de finalmente achar a menor pos. Y, transforma em modulo pra somar direito depois.
...
for element in sorted_file_list: # Leitura do trace do crawdad refinado linha a linha
	...
	if (found_negative_X_pos == True):
		converted_coordinate = (float(element[4])+smallest_X_pos)*0.3408
	else:
	...
	if (found_negative_Y_pos == True):
		converted_coordinate = (float(element[5])+smallest_Y_pos)*0.3408
	else:
