...
import glob
import sys
import random # Vai fazer um pseudo gerador de números aleatórios

'''
======================
Gerador de Trace Médio
======================
Autor: ÁLVARO SHIOKAWA ALVAREZ
Python: 3.4.3

Para utilizar este script, no terminal digite:
Intermediate_Trace_Generator.py [nome_do_trace_original_do_crawdad.txt]
...
Aqui como no gerador original, eu converto do formato do CRAWDAD para um formato aceitável, mas além disso, eu adultero os dados
do trace original. Por fim somaremos tudo em uma média pra gerar um trace intermediário, que será usado pra gerar um TVG.

Vamos supor que estamos tratando de 3 traces, um o original, e outros dois virtuais, com estes faremos a média de horários e tempos, pra
colocar. Para isto basta gerar uma random seed pra gerar os dois valores, somar com o original e dividir por 3.
...
for element in sorted_file_list: # Leitura do trace do crawdad refinado linha a linha
	...
	# 3) Escrevendo a parte <x_coord> multiplicada por 0.3408 para converter de pés para metros, além disso adiciona um fator aleatório nas coordenadas
	converted_coordinate = (float(element[4]) + random.randint(1,1000))*0.3408
	target_file.write(str(converted_coordinate) + " ") # Por fim escreve a x coordinate e um white space

	# 4) Escrevendo a parte <y_coord> multiplicada por 0.3408 para converter de pés para metros, além disso adiciona um fator aleatório nas coordenadas
	converted_coordinate = (float(element[5]) + random.randint(1,1000))*0.3408
